...
const double kIndexBarItemHeight = 20;
...
class _IndexBarState extends State<IndexBar> {
      ...
      overlayEntry = OverlayEntry(builder: (BuildContext ctx) {
        ...
        if (widget.options.indexHintPosition != null) {
          ...
          if (widget.options.indexHintAlignment == Alignment.centerRight) {
            left = MediaQuery.of(context).size.width - kIndexBarWidth - widget.options.indexHintWidth + widget.options.indexHintOffset.dx;
            ...
            left = MediaQuery.of(context).size.width / 2 - widget.options.indexHintWidth / 2 + widget.options.indexHintOffset.dx;
            top = MediaQuery.of(context).size.height / 2 - widget.options.indexHintHeight / 2 + widget.options.indexHintOffset.dy;
          }
        }
        return Positioned(
            left: left,
  ...
  Widget _buildItem(BuildContext context, int index) {
    ...
    if (localImages.contains(tag)) {
      ...
      child = getCustomFont(tag, 13, details.index == index ? AppColors.primaryColor : AppColors.getSubFontColor(context), 1, fontWeight: FontWeight.w400).marginSymmetric(vertical: 2);
      // child = Text('$tag', style: textStyle);
    }
...
class _BaseIndexBarState extends State<BaseIndexBar> {
  ...
  _triggerDragEvent(int action) {
    if (widget.hapticFeedback && (action == IndexBarDragDetails.actionDown || action == IndexBarDragDetails.actionUpdate)) {
  ...
  Widget build(BuildContext context) {
    List<Widget> children = List.generate(widget.data.length, (index) {
      Widget child = widget.itemBuilder == null ? Center(child: Text('${widget.data[index]}', style: widget.textStyle)) : widget.itemBuilder!(context, index);
      return SizedBox(
        width: widget.width,
