...
class AzListView extends StatefulWidget {
  AzListView({
    ...
    this.padding,
    this.susItemBuilder,
    this.susItemHeight = az.kSusItemHeight,
    this.susPosition,
  ...
  final List<az.ISuspensionBean> data;
...
class _AzListViewState extends State<AzListView> {
  ...
  int _getIndex(String tag) {
    for (int i = 0; i < widget.itemCount; i++) {
      az.ISuspensionBean bean = widget.data[i];
  ...
  void _positionsChanged() {
    ...
    if (positions.isNotEmpty) {
      ItemPosition itemPosition = positions.where((ItemPosition position) => position.itemTrailingEdge > 0).reduce((ItemPosition min, ItemPosition position) => position.itemTrailingEdge < min.itemTrailingEdge ? position : min);
