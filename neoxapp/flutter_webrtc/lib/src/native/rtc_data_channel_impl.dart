...
class RTCDataChannelNative extends RTCDataChannel {
  RTCDataChannelNative(this._peerConnectionId, this._label, this._dataChannelId, this._flutterId, {RTCDataChannelState? state}) {
    stateChangeStream = _stateChangeController.stream;
    messageStream = _messageController.stream;
    if (state != null) {
      _state = state;
    }
    _eventSubscription = _eventChannelFor(_peerConnectionId, _flutterId).receiveBroadcastStream().listen(eventListener, onError: errorListener);
  }
  ...
  Future<void> close() async {
    ...
    await WebRTC.invokeMethod('dataChannelClose', <String, dynamic>{'peerConnectionId': _peerConnectionId, 'dataChannelId': _flutterId});
  }
}
