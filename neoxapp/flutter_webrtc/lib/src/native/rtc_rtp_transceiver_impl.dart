...
class RTCRtpTransceiverInitNative extends RTCRtpTransceiverInit {
  RTCRtpTransceiverInitNative(TransceiverDirection direction, List<MediaStream> streams, List<RTCRtpEncoding> sendEncodings) : super(direction: direction, streams: streams, sendEncodings: sendEncodings);

  factory RTCRtpTransceiverInitNative.fromMap(Map<dynamic, dynamic> map) {
    return RTCRtpTransceiverInitNative(typeStringToRtpTransceiverDirection[map['direction']]!, (map['streams'] as List<dynamic>).map((e) => MediaStreamNative.fromMap(map)).toList(), listToRtpEncodings(map['sendEncodings']));
  }
...
class RTCRtpTransceiverNative extends RTCRtpTransceiver {
  ...
  factory RTCRtpTransceiverNative.fromMap(Map<dynamic, dynamic> map, {required String peerConnectionId}) {
    var transceiver = RTCRtpTransceiverNative(map['transceiverId'] ?? '', typeStringToRtpTransceiverDirection[map['direction']]!, map['mid'] ?? '', RTCRtpSenderNative.fromMap(map['sender'], peerConnectionId: peerConnectionId), RTCRtpReceiverNative.fromMap(map['receiver'], peerConnectionId: peerConnectionId), peerConnectionId);
    return transceiver;
  }
  ...
  Future<void> setDirection(TransceiverDirection direction) async {
    try {
      await WebRTC.invokeMethod('rtpTransceiverSetDirection', <String, dynamic>{'peerConnectionId': _peerConnectionId, 'transceiverId': _id, 'direction': typeRtpTransceiverDirectionToString[direction]});
  ...
  Future<TransceiverDirection?> getCurrentDirection() async {
    try {
      final response = await WebRTC.invokeMethod('rtpTransceiverGetCurrentDirection', <String, dynamic>{'peerConnectionId': _peerConnectionId, 'transceiverId': _id});
  ...
  Future<void> stop() async {
    try {
      await WebRTC.invokeMethod('rtpTransceiverStop', <String, dynamic>{'peerConnectionId': _peerConnectionId, 'transceiverId': _id});
  ...
  Future<void> setCodecPreferences(List<RTCRtpCodecCapability> codecs) async {
    try {
      await WebRTC.invokeMethod('setCodecPreferences', <String, dynamic>{'peerConnectionId': _peerConnectionId, 'transceiverId': _id, 'codecs': codecs.map((e) => e.toMap()).toList()});
    } on PlatformException catch (e) {
