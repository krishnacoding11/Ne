...
class RTCRtpSenderNative extends RTCRtpSender {
  RTCRtpSenderNative(this._id, this._track, this._dtmf, this._parameters, this._ownsTrack, this._peerConnectionId);

  factory RTCRtpSenderNative.fromMap(Map<dynamic, dynamic> map, {required String peerConnectionId}) {
    Map<dynamic, dynamic> trackMap = map['track'];
    return RTCRtpSenderNative(map['senderId'], (trackMap.isNotEmpty) ? MediaStreamTrackNative.fromMap(map['track'], peerConnectionId) : null, RTCDTMFSenderNative(peerConnectionId, map['senderId']), RTCRtpParameters.fromMap(map['rtpParameters']), map['ownsTrack'], peerConnectionId);
  }

  static List<RTCRtpSenderNative> fromMaps(List<dynamic> map, {required String peerConnectionId}) {
    return map.map((e) => RTCRtpSenderNative.fromMap(e, peerConnectionId: peerConnectionId)).toList();
  }
  ...
  Future<bool> setParameters(RTCRtpParameters parameters) async {
    ...
    try {
      final response = await WebRTC.invokeMethod('rtpSenderSetParameters', <String, dynamic>{'peerConnectionId': _peerConnectionId, 'rtpSenderId': _id, 'parameters': parameters.toMap()});
  ...
  Future<void> replaceTrack(MediaStreamTrack? track) async {
    try {
      await WebRTC.invokeMethod('rtpSenderReplaceTrack', <String, dynamic>{'peerConnectionId': _peerConnectionId, 'rtpSenderId': _id, 'trackId': track != null ? track.id : ''});
  ...
  @Deprecated('No need to dispose rtpSender as it is handled by peerConnection.')
  @override
  @mustCallSuper
