...
class DesktopCapturerSourceNative extends DesktopCapturerSource {
  DesktopCapturerSourceNative(this._id, this._name, this._thumbnailSize, this._type);
  factory DesktopCapturerSourceNative.fromMap(Map<dynamic, dynamic> map) {
    var sourceType = (map['type'] as String) == 'window' ? SourceType.Window : SourceType.Screen;
    var source = DesktopCapturerSourceNative(map['id'], map['name'], ThumbnailSize.fromMap(map['thumbnailSize']), sourceType);
  ...
  //ignore: close_sinks
  final StreamController<String> _onNameChanged = StreamController.broadcast(sync: true);

  @override
  StreamController<String> get onNameChanged => _onNameChanged;

  final StreamController<Uint8List> _onThumbnailChanged = StreamController.broadcast(sync: true);
...
class DesktopCapturerNative extends DesktopCapturer {
  ...
  static final DesktopCapturerNative instance = DesktopCapturerNative._internal();

  @override
  StreamController<DesktopCapturerSource> get onAdded => _onAdded;
  final StreamController<DesktopCapturerSource> _onAdded = StreamController.broadcast(sync: true);

  @override
  StreamController<DesktopCapturerSource> get onRemoved => _onRemoved;
  final StreamController<DesktopCapturerSource> _onRemoved = StreamController.broadcast(sync: true);

  @override
  StreamController<DesktopCapturerSource> get onNameChanged => _onNameChanged;
  final StreamController<DesktopCapturerSource> _onNameChanged = StreamController.broadcast(sync: true);

  @override
  StreamController<DesktopCapturerSource> get onThumbnailChanged => _onThumbnailChanged;
  final StreamController<DesktopCapturerSource> _onThumbnailChanged = StreamController.broadcast(sync: true);
  ...
  Future<Uint8List?> getThumbnail(DesktopCapturerSourceNative source) async {
    final response = await WebRTC.invokeMethod(
      ...
      <String, dynamic>{
        'sourceId': source.id,
        'thumbnailSize': {'width': source.thumbnailSize.width, 'height': source.thumbnailSize.height}
      },
    );
