...
class RTCVideoRenderer extends ValueNotifier<RTCVideoValue> implements VideoRenderer {
  ...
  set srcObject(MediaStream? stream) {
    ...
    _srcObject = stream;
    WebRTC.invokeMethod('videoRendererSetSrcObject', <String, dynamic>{'textureId': textureId, 'streamId': stream?.id ?? '', 'ownerTag': stream?.ownerTag ?? ''}).then((_) {
  ...
  void setSrcObject({MediaStream? stream, String? trackId}) {
    ...
    _srcObject = stream;
    WebRTC.invokeMethod('videoRendererSetSrcObject', <String, dynamic>{'textureId': textureId, 'streamId': stream?.id ?? '', 'ownerTag': stream?.ownerTag ?? '', 'trackId': trackId ?? '0'}).then((_) {
      value = (stream == null) ? RTCVideoValue.empty : value.copyWith(renderVideo: renderVideo);
    });
  }
