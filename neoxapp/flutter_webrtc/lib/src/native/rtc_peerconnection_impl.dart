...
class RTCPeerConnectionNative extends RTCPeerConnection {
  ...
  void eventListener(dynamic event) {
    ...
    switch (map['event']) {
      ...
      case 'onAddTrack':
        ...
        var newTrack = MediaStreamTrackNative(track['id'], track['label'], track['kind'], track['enabled'], _peerConnectionId, track['settings'] ?? {});
      ...
      case 'didOpenDataChannel':
        ...
        _dataChannel = RTCDataChannelNative(_peerConnectionId, label, dataChannelId, flutterId, state: RTCDataChannelState.RTCDataChannelOpen);
      ...
      case 'onTrack':
        ...
        onTrack?.call(RTCTrackEvent(receiver: RTCRtpReceiverNative.fromMap(map['receiver'], peerConnectionId: _peerConnectionId), streams: streams, track: MediaStreamTrackNative.fromMap(map['track'], _peerConnectionId), transceiver: transceiver));
        break;
  ...
  Future<RTCSessionDescription> createOffer([Map<String, dynamic>? constraints]) async {
    try {
      final response = await WebRTC.invokeMethod('createOffer', <String, dynamic>{'peerConnectionId': _peerConnectionId, 'constraints': constraints ?? defaultSdpConstraints});
  ...
  Future<RTCSessionDescription> createAnswer([Map<String, dynamic>? constraints]) async {
    try {
      final response = await WebRTC.invokeMethod('createAnswer', <String, dynamic>{'peerConnectionId': _peerConnectionId, 'constraints': constraints ?? defaultSdpConstraints});
  ...
  Future<List<StatsReport>> getStats([MediaStreamTrack? track]) async {
    try {
      final response = await WebRTC.invokeMethod('getStats', <String, dynamic>{'peerConnectionId': _peerConnectionId, 'trackId': track?.id});

      var stats = <StatsReport>[];
      if (response != null) {
        List<dynamic> reports = response['stats'];
        for (var report in reports) {
          stats.add(StatsReport(report['id'], report['type'], (report['timestamp'] as num).toDouble(), report['values']));
        }
      }
      return stats;
    } on PlatformException catch (e) {
  ...
  Future<RTCDataChannel> createDataChannel(String label, RTCDataChannelInit dataChannelDict) async {
    try {
      final response = await WebRTC.invokeMethod('createDataChannel', <String, dynamic>{'peerConnectionId': _peerConnectionId, 'label': label, 'dataChannelDict': dataChannelDict.toMap()});

      _dataChannel = RTCDataChannelNative(_peerConnectionId, label, response['id'], response['flutterId']);
      return _dataChannel!;
    } on PlatformException catch (e) {
  ...
  Future<List<RTCRtpSender>> getSenders() async {
    try {
      final response = await WebRTC.invokeMethod('getSenders', <String, dynamic>{'peerConnectionId': _peerConnectionId});
  ...
  Future<List<RTCRtpReceiver>> getReceivers() async {
    try {
      final response = await WebRTC.invokeMethod('getReceivers', <String, dynamic>{'peerConnectionId': _peerConnectionId});
  ...
  Future<List<RTCRtpTransceiver>> getTransceivers() async {
    try {
      final response = await WebRTC.invokeMethod('getTransceivers', <String, dynamic>{'peerConnectionId': _peerConnectionId});
  ...
  Future<bool> removeTrack(RTCRtpSender sender) async {
    try {
      final response = await WebRTC.invokeMethod('removeTrack', <String, dynamic>{'peerConnectionId': _peerConnectionId, 'senderId': sender.senderId});
  ...
  Future<RTCRtpTransceiver> addTransceiver({MediaStreamTrack? track, RTCRtpMediaType? kind, RTCRtpTransceiverInit? init}) async {
    try {
      final response = await WebRTC.invokeMethod('addTransceiver', <String, dynamic>{'peerConnectionId': _peerConnectionId, if (track != null) 'trackId': track.id, if (kind != null) 'mediaType': typeRTCRtpMediaTypetoString[kind], if (init != null) 'transceiverInit': RTCRtpTransceiverInitNative.initToMap(init)});
