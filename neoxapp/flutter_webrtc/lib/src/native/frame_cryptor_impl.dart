...
class KeyProviderImpl implements KeyProvider {
  ...
  Future<bool> setKey({
    ...
    try {
      final response = await WebRTC.invokeMethod('keyProviderSetKey', <String, dynamic>{
        'keyProviderId': _id,
        'keyIndex': index,
        'key': key,
  ...
  Future<Uint8List> ratchetKey({
    ...
    try {
      final response = await WebRTC.invokeMethod('keyProviderRatchetKey', <String, dynamic>{
        'keyProviderId': _id,
        'keyIndex': index,
        'participantId': participantId,
      });
...
class FrameCryptorFactoryImpl implements FrameCryptorFactory {
  ...
  Future<FrameCryptor> createFrameCryptorForRtpSender({
    ...
    try {
      final response = await WebRTC.invokeMethod('frameCryptorFactoryCreateFrameCryptor', <String, dynamic>{
  ...
  Future<KeyProvider> createDefaultKeyProvider(KeyProviderOptions options) async {
    try {
      final response = await WebRTC.invokeMethod('frameCryptorFactoryCreateKeyProvider', <String, dynamic>{
...
class FrameCryptorImpl extends FrameCryptor {
  FrameCryptorImpl(this._frameCryptorId, this._participantId) {
    _eventSubscription = _eventChannelFor(_frameCryptorId).receiveBroadcastStream().listen(eventListener, onError: errorListener);
  }
  ...
  Future<bool> setKeyIndex(int index) async {
    try {
      final response = await WebRTC.invokeMethod('frameCryptorSetKeyIndex', <String, dynamic>{
  ...
  Future<int> get keyIndex async {
    try {
      final response = await WebRTC.invokeMethod('frameCryptorGetKeyIndex', <String, dynamic>{
        'frameCryptorId': _frameCryptorId,
      });
  ...
  Future<bool> setEnabled(bool enabled) async {
    try {
      final response = await WebRTC.invokeMethod('frameCryptorSetEnabled', <String, dynamic>{
        'frameCryptorId': _frameCryptorId,
        'enabled': enabled,
      });
      return response['result'];
  ...
  Future<bool> get enabled async {
    try {
      final response = await WebRTC.invokeMethod('frameCryptorGetEnabled', <String, dynamic>{
        'frameCryptorId': _frameCryptorId,
      });
  ...
  Future<void> dispose() async {
    ...
    try {
      final response = await WebRTC.invokeMethod('frameCryptorDispose', <String, dynamic>{
        'frameCryptorId': _frameCryptorId,
      });
