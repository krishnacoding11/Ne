...
class FrameCryptorImpl extends FrameCryptor {
  FrameCryptorImpl(this._factory, this.worker, this._participantId, this._trackId, {this.jsSender, this.jsReceiver, required this.keyProvider});
  html.Worker worker;
  bool _enabled = false;
  int _keyIndex = 0;
  ...
  Future<bool> setEnabled(bool enabled) async {
    jsutil.callMethod(worker, 'postMessage', [
      jsutil.jsify({'msgType': 'enable', 'participantId': participantId, 'enabled': enabled})
    ]);
    _enabled = enabled;
    return true;
  }
...
class KeyProviderImpl implements KeyProvider {
    ...
    jsutil.callMethod(worker, 'postMessage', [
      jsutil.jsify({
        ...
        'keyOptions': {
          ...
          if (options.uncryptedMagicBytes != null) 'uncryptedMagicBytes': base64Encode(options.uncryptedMagicBytes!),
        },
      })
    ]);
  }
  ...
  @override
  Future<bool> setKey({required String participantId, required int index, required Uint8List key}) async {
    jsutil.callMethod(worker, 'postMessage', [
      jsutil.jsify({
        'msgType': 'setKey',
        'participantId': participantId,
...
class FrameCryptorFactoryImpl implements FrameCryptorFactory {
  FrameCryptorFactoryImpl._internal() {
    ...
    worker.onMessage.listen((msg) {
      ...
      if (type == 'cryptorState') {
        ...
        var frameCryptor = _frameCryptors.values.firstWhereOrNull((element) => (element as FrameCryptorImpl).trackId == trackId);
        var state = msg.data['state'];
        FrameCryptorState frameCryptorState = FrameCryptorState.FrameCryptorStateNew;
        switch (state) {
          case 'ok':
            frameCryptorState = FrameCryptorState.FrameCryptorStateOk;
            break;
          case 'decryptError':
            frameCryptorState = FrameCryptorState.FrameCryptorStateDecryptionFailed;
            break;
          case 'encryptError':
            frameCryptorState = FrameCryptorState.FrameCryptorStateEncryptionFailed;
            break;
          case 'missingKey':
            frameCryptorState = FrameCryptorState.FrameCryptorStateMissingKey;
            break;
          case 'internalError':
            frameCryptorState = FrameCryptorState.FrameCryptorStateInternalError;
            break;
          case 'keyRatcheted':
            frameCryptorState = FrameCryptorState.FrameCryptorStateKeyRatcheted;
            break;
        }
        frameCryptor?.onFrameCryptorStateChanged?.call(participantId, frameCryptorState);
      } else if (type == 'ratchetKey') {
        var trackId = msg.data['trackId'];
        var frameCryptor = _frameCryptors.values.firstWhereOrNull((element) => (element as FrameCryptorImpl).trackId == trackId);
        if (frameCryptor != null) {
          (frameCryptor as FrameCryptorImpl).keyProvider.onRatchetKey(base64Decode(msg.data['key']));
        }
      }
    });
  ...
  static final FrameCryptorFactoryImpl instance = FrameCryptorFactoryImpl._internal();
  ...
  @override
  Future<FrameCryptor> createFrameCryptorForRtpReceiver({required String participantId, required RTCRtpReceiver receiver, required Algorithm algorithm, required KeyProvider keyProvider}) {
    html.RtcRtpReceiver jsReceiver = (receiver as RTCRtpReceiverWeb).jsRtpReceiver;
    ...
    if (js.context['RTCRtpScriptTransform'] != null) {
      ...
      jsutil.setProperty(jsReceiver, 'transform', RTCRtpScriptTransform(worker, jsutil.jsify(options)));
    } else {
      var writable = jsReceiver.writable;
      var readable = jsReceiver.readable;
      bool exist = true;
      if (writable == null || readable == null) {
        EncodedStreams streams = jsutil.callMethod(jsReceiver, 'createEncodedStreams', []);
      ...
      jsutil.callMethod(worker, 'postMessage', [
        jsutil.jsify({'msgType': 'decode', 'kind': kind, 'exist': exist, 'participantId': participantId, 'trackId': trackId, 'readableStream': readable, 'writableStream': writable}),
        jsutil.jsify([readable, writable]),
      ]);
    }
    FrameCryptor cryptor = FrameCryptorImpl(this, worker, participantId, trackId, jsReceiver: jsReceiver, keyProvider: keyProvider as KeyProviderImpl);
    _frameCryptors[trackId] = cryptor;
    return Future.value(cryptor);
  }

  @override
  Future<FrameCryptor> createFrameCryptorForRtpSender({required String participantId, required RTCRtpSender sender, required Algorithm algorithm, required KeyProvider keyProvider}) {
    ...
    if (js.context['RTCRtpScriptTransform'] != null) {
      ...
      jsutil.setProperty(jsSender, 'transform', RTCRtpScriptTransform(worker, jsutil.jsify(options)));
    } else {
      var writable = jsSender.writable;
      var readable = jsSender.readable;
      bool exist = true;
      if (writable == null || readable == null) {
        EncodedStreams streams = jsutil.callMethod(jsSender, 'createEncodedStreams', []);
        readable = streams.readable;
      ...
      jsutil.callMethod(worker, 'postMessage', [
        jsutil.jsify({'msgType': 'encode', 'kind': kind, 'exist': exist, 'participantId': participantId, 'trackId': trackId, 'options': (keyProvider as KeyProviderImpl).options.toJson(), 'readableStream': readable, 'writableStream': writable}),
        jsutil.jsify([readable, writable]),
      ]);
    }
    FrameCryptor cryptor = FrameCryptorImpl(this, worker, participantId, trackId, jsSender: jsSender, keyProvider: keyProvider);
    _frameCryptors[trackId] = cryptor;
    return Future.value(cryptor);
  }
