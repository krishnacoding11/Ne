...
IncomingMessage? parseMessage(String data, UA? ua) {
  ...
  if (parsed == -1) {
    logger.e('parseMessage() | error parsing first line of SIP message: "$firstLine"');
...
int getHeader(String data, int headerStart) {
  ...
  while (end == 0) {
    ...
    if (!data.substring(partialEnd + 2, partialEnd + 4).contains(RegExp(r'(^\r\n)')) && String.fromCharCode(data.codeUnitAt(partialEnd + 2)).contains(RegExp(r'(^\s+)'))) {
...
dynamic parseHeader(IncomingMessage message, String data, int headerStart, int headerEnd) {
  dynamic parsed;
