...
class SIPUAHelper extends EventManager {
  ...
  Call? findCallNumber(String number) {
    if (_calls.isNotEmpty) {
      if (_calls.values.first.remote_identity == number) {
        return _calls.values.first;
      } else {
        return _calls.values.first;
      }
    }
...
class Call {
  ...
  void answer(Map<String, dynamic> options, {MediaStream? mediaStream = null}) {
    try {
      ...
      print('Instance of InvalidStateError  answer${e.toString()}');
    }
  }
  ...
  void refer(String target) {
    ...
    // }catch(e){
    //   print('transfer Error ${e.toString()}');
    // }
  }
