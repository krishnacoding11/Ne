...
bool isDecimal(dynamic input) => input != null && (input is num && !input.isNaN || (input is! num && (double.tryParse(input) != null || int.tryParse(input, radix: 10) != null)));
...
*/
String escapeUser(String user) => Uri.encodeComponent(decodeURIComponent(user)).replaceAll(RegExp(r'%3A', caseSensitive: false), ':').replaceAll(RegExp(r'%2B', caseSensitive: false), '+').replaceAll(RegExp(r'%3F', caseSensitive: false), '?').replaceAll(RegExp(r'%2F', caseSensitive: false), '/');
...
URI? normalizeTarget(dynamic target, [String? domain]) {
  ...
  if (target == null) {
    ...
    targetUser = targetUser.replaceAll(RegExp(r'^(sips?|tel):', caseSensitive: false), '');
...
String headerize(String str) {
  Map<String, String> exceptions = <String, String>{'Call-Id': 'Call-ID', 'Cseq': 'CSeq', 'Www-Authenticate': 'WWW-Authenticate'};
  ...
  for (part = 0; part < parts; part++) {
    ...
    hname += String.fromCharCodes(<int>[names[part].codeUnitAt(0)]).toUpperCase() + names[part].substring(1);
  }
