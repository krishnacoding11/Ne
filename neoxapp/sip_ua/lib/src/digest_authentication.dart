...
class Challenge {
  Challenge(this.algorithm, this.realm, this.nonce, this.opaque, this.stale, this.qop);
  factory Challenge.fromMap(Map<String, dynamic> map) {
    return Challenge(map['algorithm'], map['realm'], map['nonce'], map['opaque'], map['stale'], map['qop']);
  }
  String? algorithm;
  String? realm;
  String? nonce;
...
class Credentials {
  ...
  factory Credentials.fromMap(Map<String, dynamic> map) {
    return Credentials(map['username'], map['password'], map['realm'], map['ha1']);
  }
  String? username;
  String? password;
  String? realm;
...
*/
  bool authenticate(SipMethod method, Challenge challenge, [dynamic ruri, String? cnonce, String? body]) {
    ...
    if (_algorithm != null) {
      if (_algorithm != 'MD5') {
        logger.e('authenticate() | challenge with Digest algorithm different than "MD5", authentication aborted');
    ...
    if (_nonce == null) {
      logger.e('authenticate() | challenge without Digest nonce, authentication aborted');

      return false;
    }

    if (_realm == null) {
      logger.e('authenticate() | challenge without Digest realm, authentication aborted');
    ...
    if (_credentials.password == null) {
      ...
      if (_credentials.ha1 == null) {
        logger.e('authenticate() | no plain SIP password nor ha1 provided, authentication aborted');
      ...
      if (_credentials.realm != _realm) {
        logger.e('authenticate() | no plain SIP password, and stored "realm" does not match the given "realm", cannot authenticate [stored:"${_credentials.realm}", given:"$_realm"]');
    ...
    if (challenge.qop != null && challenge.qop.isNotEmpty) {
      if (challenge.qop.indexOf('auth-int') > -1) {
        ...
        logger.e('authenticate() | challenge without Digest qop different than "auth" or "auth-int", authentication aborted');
    ...
    if (_credentials.password != null) {
      ...
      _ha1 = utils.calculateMD5('${_credentials.username}:$_realm:${_credentials.password}');
    }
    ...
    if (_qop == 'auth') {
      ...
      _response = utils.calculateMD5('$_ha1:$_nonce:$_ncHex:$_cnonce:auth:$ha2');
    } else if (_qop == 'auth-int') {
      // HA2 = MD5(A2) = MD5(method:digestURI:MD5(entityBody)).
      a2 = '${SipMethodHelper.getName(_method)}:$_uri:${utils.calculateMD5(body ?? '')}';
      ha2 = utils.calculateMD5(a2);

      logger.d('authenticate() | using qop=auth-int [a2:$a2]');

      // Response = MD5(HA1:nonce:nonceCount:credentialsNonce:qop:HA2).
      _response = utils.calculateMD5('$_ha1:$_nonce:$_ncHex:$_cnonce:auth-int:$ha2');
    } else if (_qop == null) {
      // HA2 = MD5(A2) = MD5(method:digestURI).
      a2 = '${SipMethodHelper.getName(_method)}:$_uri';
      ha2 = utils.calculateMD5(a2);
...
*/
  ...
  String toString() {
    ...
    if (_response == null) {
      throw AssertionError('response field does not exist, cannot generate Authorization header');
    }
