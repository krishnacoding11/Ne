...
class URI {
  ...
  void setHeader(String name, dynamic value) {
    _headers[utils.headerize(name)] = (value is List) ? value : <dynamic>[value];
  }
  ...
  URI clone() {
    return URI(scheme, user, host, port, decoder.convert(encoder.convert(_parameters)), decoder.convert(encoder.convert(_headers)));
  }
