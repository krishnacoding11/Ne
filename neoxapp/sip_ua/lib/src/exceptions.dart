...
class ConfigurationError extends ErrorImpl {
  ConfigurationError(String parameter, [dynamic value]) : super(1, 'CONFIGURATION_ERROR', (value == null) ? 'Missing parameter: $parameter' : 'Invalid value ${encoder.convert(value)} for parameter "$parameter"', parameter: parameter, value: value);
}

class InvalidStateError extends ErrorImpl {
  InvalidStateError(dynamic status) : super(2, 'INVALID_STATE_ERROR', 'Invalid status: $status', status: status);
}
