  ...
  Log() : super(printer: MyLogPrinter('.')) {
    ...
    if (frames.frames != null) {
      for (Stackframe frame in frames.frames!) {
        _localPath = frame.sourceFile.path.substring(frame.sourceFile.path.lastIndexOf('/'));
        break;
      }
    }
  }
...
class MyLogPrinter extends LogPrinter {
  ...
  List<String> log(LogEvent event) {
    ...
    print(color('[$formattedDate] ${event.level} ${StackTraceNJ(skipFrames: depth).formatStackTrace(methodCount: 1)} ::: ${event.message}'));
    if (event.error != null) {
      print('${event.error}');
    }
