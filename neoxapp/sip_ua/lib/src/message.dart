...
class Message extends EventManager with Applicant {
  ...
  void send(String target, String body, [Map<String, dynamic>? options, Map<String, dynamic>? params]) {
    ...
    _request = OutgoingRequest(SipMethod.MESSAGE, normalized, _ua, params, extraHeaders);
    if (body != null) {
      _request.body = body;
    }
  ...
  void _receiveResponse(IncomingResponse? response) {
    ...
    } else if (RegExp(r'^2[0-9]{2}$').hasMatch(response.status_code.toString())) {
  ...
  void _onRequestTimeout() {
    ...
    _failed('system', 408, DartSIP_C.CausesType.REQUEST_TIMEOUT, 'Request Timeout');
  }
  ...
  void _onTransportError() {
    ...
    _failed('system', 500, DartSIP_C.CausesType.CONNECTION_ERROR, 'Transport Error');
  }
  ...
  void _failed(String originator, int? status_code, String cause, String? reason_phrase) {
    logger.d('MESSAGE failed');
    close();
    logger.d('emit "failed"');
    emit(EventCallFailed(originator: originator, cause: ErrorCause(cause: cause, status_code: status_code, reason_phrase: reason_phrase)));
  }
