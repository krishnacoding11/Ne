...
class EventNewRTCSession extends CallEvent {
  EventNewRTCSession({RTCSession? session, String? originator, dynamic request}) : super(session);
  String? originator;
  dynamic request;
}
...
class EventCallEnded extends CallEvent {
  EventCallEnded({RTCSession? session, this.originator, this.cause, this.request}) : super(session);
  String? originator;
  ErrorCause? cause;
...
class EventCallProgress extends CallEvent {
  EventCallProgress({RTCSession? session, this.originator, this.response, this.cause}) : super(session);
  String? originator;
  dynamic response;
  ErrorCause? cause;
}
...
class EventCallConfirmed extends CallEvent {
  EventCallConfirmed({RTCSession? session, this.originator, this.ack}) : super(session);
  String? originator;
  dynamic ack;
}
...
class EventCallUnmuted extends CallEvent {
  EventCallUnmuted({RTCSession? session, this.audio, this.video}) : super(session);
  bool? audio;
  bool? video;
}

class EventCallAccepted extends CallEvent {
  EventCallAccepted({RTCSession? session, this.originator, this.response}) : super(session);
  String? originator;
  dynamic response;
}

class EventCallFailed extends CallEvent {
  EventCallFailed({RTCSession? session, String? state, this.response, this.originator, MediaStream? stream, this.cause, this.request, this.status_line}) : super(session);
  dynamic response;
  String? originator;
  ErrorCause? cause;
  dynamic request;
...
class EventStream extends CallEvent {
  EventStream({RTCSession? session, this.originator, this.stream}) : super(session);
  String? originator;
  MediaStream? stream;
}

class EventCallRefer extends CallEvent {
  EventCallRefer({RTCSession? session, this.aor, this.accept, this.reject}) : super(session);
  String? aor;
