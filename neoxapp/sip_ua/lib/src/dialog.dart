...
class Dialog {
  Dialog(this._owner, dynamic message, String type, [int? state]) {
    ...
    if (!message.hasHeader('contact')) {
      throw Exceptions.TypeError('unable to create a Dialog without Contact header field');
    }

    if (message is IncomingResponse) {
      state = (message.status_code < 200) ? DialogStatus.STATUS_EARLY : DialogStatus.STATUS_CONFIRMED;
    }
  ...
  // RFC 3261 12.2.1.1.
  OutgoingRequest _createRequest(SipMethod method, List<dynamic> extraHeaders, String? body) {
    extraHeaders = Utils.cloneArray(extraHeaders);

    local_seqnum ??= (Utils.Math.randomDouble() * 10000).floor();

    num? cseq = (method == SipMethod.CANCEL || method == SipMethod.ACK) ? local_seqnum : local_seqnum = local_seqnum! + 1;

    OutgoingRequest request = OutgoingRequest(method, _remote_target, _ua, <String, dynamic>{'cseq': cseq, 'call_id': _id!.call_id, 'from_uri': _local_uri, 'from_tag': _id!.local_tag, 'to_uri': _remote_uri, 'to_tag': _id!.remote_tag, 'route_set': _route_set}, extraHeaders, body);
  ...
  bool _checkInDialogRequest(IncomingRequest request) {
    ...
    if (request.method == SipMethod.INVITE || (request.method == SipMethod.UPDATE && request.body != null)) {
      if (uac_pending_reply == true) {
        ...
        stateChanged = (EventStateChanged state) {
          if (request.server_transaction!.state == TransactionState.ACCEPTED || request.server_transaction!.state == TransactionState.COMPLETED || request.server_transaction!.state == TransactionState.TERMINATED) {
