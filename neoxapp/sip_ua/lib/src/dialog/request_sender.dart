...
class DialogRequestSender {
  ...
  void send() {
    ...
    if ((_request.method == SipMethod.INVITE || (_request.method == SipMethod.UPDATE && _request.body != null)) && request_sender.clientTransaction?.state != TransactionState.TERMINATED) {
      ...
      stateChanged = (EventStateChanged data) {
        if (request_sender.clientTransaction?.state == TransactionState.ACCEPTED || request_sender.clientTransaction?.state == TransactionState.COMPLETED || request_sender.clientTransaction?.state == TransactionState.TERMINATED) {
  ...
  void _receiveResponse(IncomingResponse response) {
    ...
    } else if (response.method == SipMethod.INVITE && response.status_code == 491) {
      if (_reattempt != null) {
