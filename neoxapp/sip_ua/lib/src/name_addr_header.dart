...
class NameAddrHeader {
  NameAddrHeader(URI? uri, String? display_name, [Map<dynamic, dynamic>? parameters]) {
    // Checks.
    if (uri == null) {
  ...
  void setParam(String? key, dynamic value) {
    if (key != null) {
      _parameters![key.toLowerCase()] = (value == null) ? null : value.toString();
    }
  }
  ...
  NameAddrHeader clone() {
    return NameAddrHeader(_uri!.clone(), _display_name, decoder.convert(encoder.convert(_parameters)));
  }
  ...
  @override
  String toString() {
    String body = (_display_name != null && _display_name!.length > 0) ? '"${_quote(_display_name!)}" ' : '';
