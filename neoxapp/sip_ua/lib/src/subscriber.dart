...
class Subscriber extends EventManager implements Owner {
  Subscriber(this.ua, this._target, String eventName, String accept, [int expires = 900, String? contentType, String? allowEvents, Map<String, dynamic> requestParams = const <String, dynamic>{}, List<String> extraHeaders = const <String>[]]) {
    logger.d('new');
  ...
  void receiveNotifyRequest(IncomingRequest request) {
    ...
    if (prevState != C.STATE_TERMINATED && newState != C.STATE_TERMINATED) {
      ...
      if (subsState.expires != null) {
        ...
        if (_expires_timestamp!.difference(expiresTimestamp) > Duration(milliseconds: maxTimeDeviation)) {
  ...
  void _dialogTerminated(int terminationCode, [String? reason, int? retryAfter]) {
    ...
    emit(EventTerminated(TerminationCode: terminationCode, reason: reason, retryAfter: retryAfter));
  }
  ...
  void _sendInitialSubscribe(String? body, List<dynamic> headers) {
    ...
    OutgoingRequest request = OutgoingRequest(SipMethod.SUBSCRIBE, ua.normalizeTarget(_target)!, ua, _params, headers, body);
